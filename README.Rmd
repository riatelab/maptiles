...
![](https://cranlogs.r-pkg.org/badges/grand-total/maptiles)
...
**Note:** `maptiles` uses [`terra`](https://github.com/rspatial/terra) which requires a recent version of GDAL (>= 3.0.4).  
...
```{r example,fig.ext="png", margin = TRUE, fig.width=827/96, fig.height = 318/96, dpi = 96, fig.show='hide'}
library(sf)
library(maptiles)
...
```{r example2,fig.ext="png", margin = TRUE, fig.width=827/96, fig.height = 318/96, fig.show='hide', dpi = 96}
...
on the maps. 
`get_credit()` displays a short credit text to add on each map 
using the downloaded tiles. 


## Background

Most of `maptiles`code comes from `getTiles()` and `tilesLayer()` functions in 
[`cartography`](https://github.com/riatelab/cartography). It appears useful to 
me to have a package focused on the download and display of map tiles only. 
On the technical side, it uses `terra` instead of `raster` for managing raster 
data.
